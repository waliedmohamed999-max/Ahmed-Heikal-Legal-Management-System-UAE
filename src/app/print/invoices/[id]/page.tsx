...
export default async function PrintInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-dvh bg-surface-muted py-8 print:bg-white print:py-0">
      ...
      <article className="mx-auto max-w-[800px] bg-white p-10 text-body text-[#121826] shadow-md print:max-w-none print:p-0 print:shadow-none">
        <header className="flex items-start justify-between gap-6 border-b-2 border-[#0e1b33] pb-6">
          <div className="flex items-center gap-3 text-[#0e1b33]">
            <Logo size={44} />
            <div>
              <p className="text-heading font-semibold">{o.name}</p>
              {o.nameAr && <p dir="rtl" className="text-ui">{o.nameAr}</p>}
              {o.address && <p className="text-meta text-[#4a5163]">{o.address}</p>}
              {o.trn && <p className="ltr-nums text-meta text-[#4a5163]">{t("finance.trn")}: {o.trn}</p>}
            </div>
          </div>
          <div className="text-end">
            <p className="text-[20px] font-semibold tracking-tight">{t("finance.taxInvoice")}{locale === "en" ? " / فاتورة ضريبية" : " / Tax Invoice"}</p>
            <p className="ltr-nums mt-1 font-mono text-ui">{inv.number}</p>
            <p className="mt-2 text-meta text-[#4a5163]">{t("finance.issueDate")}: {formatDate(inv.issueDate, locale, tz)}</p>
            <p className="text-meta text-[#4a5163]">{t("finance.dueDate")}: {formatDate(inv.dueDate, locale, tz)}</p>
          </div>
        </header>
        <section className="mt-6 grid grid-cols-2 gap-6">
          <div>
            <p className="text-caption font-semibold uppercase tracking-wide text-[#7b8191]">{t("finance.billTo")}</p>
          ...
          {inv.matter && (
            <div>
              <p className="text-caption font-semibold uppercase tracking-wide text-[#7b8191]">{t("finance.matter")}</p>
        ...
        <dl className="ms-auto mt-6 w-72 space-y-1.5">
          ...
          <div className="flex justify-between border-t-2 border-[#0e1b33] pt-2 text-heading font-semibold"><dt>{t("finance.total")}</dt><dd className="ltr-nums tabular">{formatMoney(inv.total, locale, cur)}</dd></div>
        ...
        {inv.notes && <p className="mt-8 whitespace-pre-line border-t border-[#ebebe6] pt-4 text-meta text-[#4a5163]">{inv.notes}</p>}
        {o.isDemo && <p className="mt-8 text-center text-caption text-[#b42318]">{t("app.demoBanner")}</p>}
      </article>
    </div>
  );
}
