...
import { Page, PageHeader, Panel, EmptyState, LinkTabs } from "@/components/ui/layout";
import { Button } from "@/components/ui/button";
import { StatusText, Badge, INVOICE_STATUS_TONE } from "@/components/ui/badge";
          ...
          {rows.map((i) => {
            ...
            return (
              <TR key={i.id}>
                ...
                <TD className="ltr-nums hidden font-mono text-meta text-ink-muted md:table-cell">{i.matter?.internalNumber ?? "—"}</TD>
                <TD className={`hidden whitespace-nowrap sm:table-cell ${late ? "font-medium text-danger" : "text-ink-muted"}`}>{formatDate(i.dueDate, locale, tz)}</TD>
                ...
                <TD><StatusText tone={late ? "danger" : INVOICE_STATUS_TONE[i.status]}>{late ? t("finance.overdue") : t(`enums.invoiceStatus.${i.status}`)}</StatusText></TD>
              </TR>
            );
          })}
        </tbody>
    ...
    <Page width="full" className="max-w-[1440px]">
      ...
      {tab === "overview" && snap && (
        <div className="mt-5 space-y-5">
          <dl className="grid grid-cols-2 gap-x-8 gap-y-3 border-y border-line py-3 lg:grid-cols-4">
            {[[t("finance.outstanding"), snap.outstanding, `${snap.outstandingCount}`, ""], [t("finance.overdue"), snap.overdue, `${snap.overdueCount}`, snap.overdue > 0 ? "text-danger" : ""], [t("finance.receivedMonth"), snap.receivedThisMonth, "", "text-success"], [t("finance.unbilled"), snap.unbilledValue, formatMinutes(snap.unbilledMinutes, locale), ""]].map(([k, v, sub, c]) => (
              <div key={k as string} className="min-w-0">
                ...
                <dd className={`mt-0.5 text-[18px] font-semibold leading-7 tabular ${c}`}><span className="ltr-nums">{formatMoney(v as number, locale, cur)}</span>{sub && <span className="ms-2 text-meta font-normal text-ink-subtle">{sub}</span>}</dd>
              </div>
            ))}
          </dl>
            ...
            <Panel className="xl:col-span-4" title={t("finance.recentPayments")} icon={<CreditCard />}>
              {data.payments.length === 0 ? <EmptyState compact title={t("finance.noPayments")} /> : (
                <ul className="divide-y divide-line">
                  {data.payments.map((p) => (
                    <li key={p.id} className="flex items-center justify-between gap-2 px-4 py-2.5 text-body">
                      <div className="min-w-0"><Link href={`/app/finance/invoices/${p.invoice.id}`} className="ltr-nums font-mono hover:underline">{p.invoice.number}</Link><p className="truncate text-meta text-ink-subtle">{L(p.invoice.client.nameEn, p.invoice.client.nameAr)} · {formatDate(p.receivedAt, locale, tz)}</p></div>
        ...
        <div className="mt-5">
          <div className="mb-3 flex flex-wrap gap-1">
            {["", "DRAFT", "ISSUED", "PARTIALLY_PAID", "PAID", "OVERDUE", "VOID"].map((s) => (
              <Link key={s} href={`/app/finance?tab=invoices${s ? `&status=${s}` : ""}`} className={`inline-flex h-7 items-center rounded-md px-2.5 text-body transition-colors ${(sp.status ?? "") === s ? "bg-surface-sunken font-medium text-ink" : "text-ink-muted hover:bg-surface-muted hover:text-ink"}`}>
                {s ? (s === "OVERDUE" ? t("finance.overdue") : t(`enums.invoiceStatus.${s}`)) : t("common.all")}
              </Link>
            ))}
          </div>
          <div className="overflow-hidden rounded-lg border border-line bg-surface">{invoiceTable(data.invoices)}</div>
        </div>
      )}
              ...
              <tbody>
                {data.expenses.map((e) => (
                  <TR key={e.id}>
                    ...
                    <TD className="ltr-nums font-mono text-meta">{e.matter ? <Link href={`/app/cases/${e.matter.id}/finance`} className="hover:underline">{e.matter.internalNumber}</Link> : "—"}</TD>
              ...
              <tbody>
                {data.time.map((e) => (
                  <TR key={e.id}>
                    ...
                    <TD className="ltr-nums font-mono text-meta"><Link href={`/app/cases/${e.matter.id}/finance`} className="hover:underline">{e.matter.internalNumber}</Link></TD>
