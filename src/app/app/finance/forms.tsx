    ...
    <form onSubmit={submit} className="space-y-5" noValidate>
      ...
      <div className="rounded-lg border border-line bg-surface shadow-xs">
        <div className="flex items-center justify-between border-b border-line px-4 py-2.5">
          <h2 className="text-[13px] font-semibold">{t("finance.items")}</h2>
        ...
        <div className="divide-y divide-line">
          {items.fields.map((f, i) => (
            <div key={f.id} className="grid gap-2 px-4 py-2.5 sm:grid-cols-[1fr_130px_90px_130px_120px_auto] sm:items-center">
              ...
              <span className="ltr-nums text-end text-[13px] tabular">{formatMoney(Number(w.items?.[i]?.quantity || 0) * Number(w.items?.[i]?.unitPrice || 0), locale)}</span>
        ...
        <div className="flex items-start justify-between gap-4 border-t border-line px-4 py-3">
          ...
          <dl className="w-64 space-y-1 text-[13px]">
            ...
            <div className="flex justify-between border-t border-line pt-1 text-[14px] font-semibold"><dt>{t("finance.total")}</dt><dd className="ltr-nums tabular">{formatMoney(total, locale)}</dd></div>
          </dl>
        </div>
      </div>
...
export function TimerWidget({ matterId, running }: { matterId: string; running: { id: string; startedAt: string; activity: string } | null }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {running ? (
        <>
          <span className="inline-flex items-center gap-2 rounded-md bg-info-soft px-2.5 py-1.5 text-[13px] font-medium text-info"><Clock3 className="size-4 animate-pulse-soft" /> {t(`finance.time.activities.${running.activity}`)} <span className="ltr-nums font-mono tabular" suppressHydrationWarning>{hh}:{mm}:{ss}</span></span>
        ...
        <>
          <Select value={activity} onChange={(e) => setActivity(e.target.value as never)} className="h-8 w-40 text-[13px]" aria-label={t("finance.time.activity")}>{TIME_ACTIVITIES.map((a) => <option key={a} value={a}>{t(`finance.time.activities.${a}`)}</option>)}</Select>
