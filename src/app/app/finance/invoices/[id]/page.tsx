...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-4 pb-10 pt-5 sm:px-6 lg:px-8 lg:pt-6">
      <Link href="/app/finance?tab=invoices" className="inline-flex items-center gap-1 text-meta text-ink-subtle hover:text-ink"><Back className="size-3.5" /> {t("finance.tabs.invoices")}</Link>
      ...
      {inv.status !== "DRAFT" && <p className="mt-4 flex items-center gap-1.5 text-meta text-ink-subtle"><Lock className="size-3.5" /> {t("finance.lockedNote")}</p>}
      <div className="mt-5 grid grid-cols-1 gap-5 lg:grid-cols-3">
        <Panel className="lg:col-span-2" title={t("finance.items")}>
          ...
          <dl className="ms-auto w-full max-w-xs space-y-1 border-t border-line p-4 text-body">
            ...
            <div className="flex justify-between text-ui font-semibold"><dt>{t("finance.due")}</dt><dd className="ltr-nums tabular">{formatMoney(due, locale, cur)}</dd></div>
          </dl>
        </Panel>
          ...
          <Panel title={t("finance.history")} icon={<CreditCard />}>
            {inv.payments.length === 0 ? <EmptyState compact title={t("finance.noPayments")} /> : (
              <ul className="divide-y divide-line">
                {inv.payments.map((p) => (
                  <li key={p.id} className="flex items-center justify-between px-4 py-2.5 text-body">
                    <div><p>{t(`finance.methods.${p.method}`)}{p.reference && <span className="ltr-nums text-ink-subtle"> · {p.reference}</span>}</p><p className="text-meta text-ink-subtle">{formatDate(p.receivedAt, locale, tz)}</p></div>
