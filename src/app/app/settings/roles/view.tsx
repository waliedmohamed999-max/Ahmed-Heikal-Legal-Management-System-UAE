    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[260px_1fr]">
      <Panel title={t("settings.roles.title")}>
        <ul className="p-1.5">
          {roles.map((r) => (
            <li key={r.id}>
              <button type="button" onClick={() => pick(r.id)} className={cn("flex w-full items-center justify-between gap-2 rounded-md px-2.5 py-2 text-start text-body", selId === r.id ? "bg-surface-muted font-medium" : "hover:bg-surface-muted/60")}>
                <span className="truncate">{r.label}</span>
                <span className="text-caption text-ink-subtle tabular">{r.users}</span>
              </button>
            </li>
          ))}
          <li><button type="button" onClick={() => pick("new")} className={cn("mt-1 flex w-full items-center gap-1.5 rounded-md px-2.5 py-2 text-body text-accent", selId === "new" && "bg-surface-muted")}><Plus className="size-4" /> {t("settings.roles.new")}</button></li>
        </ul>
      </Panel>
          ...
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {modules.map(([mod, perms]) => (
              <fieldset key={mod} className="rounded-md border border-line p-3">
                <legend className="px-1 text-meta font-semibold uppercase tracking-wide text-ink-subtle">{mod}</legend>
                <div className="space-y-1">
                  {perms.map((p) => (
                    <label key={p.key} className="flex cursor-pointer items-start gap-2 rounded px-1 py-0.5 text-meta hover:bg-surface-muted">
                      <input type="checkbox" checked={has(p.key)} onChange={() => toggle(p.key)} className="mt-0.5 accent-[var(--accent)]" />
                      <span><span className="font-mono text-meta text-ink" dir="ltr">{p.key}</span><span className="block text-meta text-ink-subtle" dir="ltr">{p.description}</span></span>
                    </label>
                  ))}
                </div>
              </fieldset>
