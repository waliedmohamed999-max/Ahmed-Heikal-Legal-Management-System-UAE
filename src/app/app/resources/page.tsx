                ...
                <li key={r.id}>
                  <a href={r.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-4 py-3 hover:bg-surface-muted/60">
                    <div className="min-w-0 flex-1">
                      <p className="text-[13.5px] font-medium text-ink">{locale === "ar" ? r.titleAr || r.title : r.title}</p>
                      <p className="ltr-nums truncate text-[12px] text-ink-subtle">{r.url.replace(/^https?:\/\//, "")}</p>
                    </div>
