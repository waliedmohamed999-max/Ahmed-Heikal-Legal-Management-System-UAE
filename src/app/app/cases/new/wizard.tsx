    ...
    <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[220px_1fr]">
        ...
        {STEPS.map((s, i) => (
          <li key={s} className="shrink-0">
            <button type="button" onClick={() => i < step && setStep(i)} disabled={i > step} aria-current={i === step ? "step" : undefined}
              className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-start text-body transition-colors",
                i === step ? "bg-surface font-medium text-ink shadow-xs ring-1 ring-line" : i < step ? "text-ink-muted hover:bg-surface" : "text-ink-subtle")}>
              <span className={cn("flex size-5 shrink-0 items-center justify-center rounded-full text-caption font-semibold tabular",
      ...
      <div className="rounded-lg border border-line bg-surface">
        <div className="border-b border-line px-5 py-3.5">
          <p className="text-meta font-medium text-ink-subtle tabular">{step + 1} / {STEPS.length}</p>
          <h2 className="text-heading font-semibold text-ink">{t(`intake.steps.${current}`)}</h2>
        </div>
        ...
        <div className="min-h-[320px] p-5">
              ...
              <div className="inline-flex rounded-md border border-line p-0.5" role="radiogroup">
                {(["existing", "new"] as const).filter((m) => m === "existing" || canCreateClient).map((m) => (
                  <button key={m} type="button" role="radio" aria-checked={clientMode === m} onClick={() => setClientMode(m)}
                    className={cn("rounded px-3 py-1.5 text-body font-medium", clientMode === m ? "bg-brand text-brand-fg" : "text-ink-muted")}>
          ...
          {current === "opponent" && (
            <div className="space-y-3">
              {parties.fields.length === 0 && <p className="text-body text-ink-muted">{t("intake.noParties")}</p>}
          ...
          {current === "jurisdiction" && (
            <div className="space-y-4">
              ...
              <p className="rounded-md bg-info-soft px-3 py-2.5 text-meta text-info">{t("intake.jurisdictionNote")}</p>
            </div>
          )}
          ...
          {current === "documents" && (
            <div className="flex flex-col items-center justify-center gap-4 py-8 text-center">
              ...
              <p className="max-w-md text-body text-ink-muted">{t("intake.documentsNote")}</p>
          ...
          {current === "conflict" && (
            <div className="space-y-4">
              <p className="text-body text-ink-muted">{t("intake.conflictIntro")}</p>
              ...
              {matches && (
                matches.length === 0 ? (
                  <p className="flex items-center gap-2 rounded-md bg-success-soft px-3 py-2.5 text-body text-success"><ShieldCheck className="size-4" /> {t("intake.noMatches")}</p>
                ) : (
                  <div className="rounded-md border border-warning/30">
                    <p className="flex items-center gap-2 border-b border-warning/30 bg-warning-soft px-3 py-2 text-body font-medium text-warning"><AlertTriangle className="size-4" /> {t("intake.potentialMatches", { n: matches.length })}</p>
                    <ul className="max-h-64 divide-y divide-line overflow-y-auto scrollbar-thin">
                      {matches.map((m, i) => (
                        <li key={i} className="flex flex-wrap items-center gap-2 px-3 py-2 text-body">
                          ...
                          <span className="text-meta text-ink-subtle">“{m.term}” · {Math.round(m.similarity * 100)}% {t("intake.similarity")}</span>
                          {m.matter && (
                            m.matter.id ? (
                              <a href={`/app/cases/${m.matter.id}`} target="_blank" rel="noreferrer" className="ms-auto text-meta text-accent hover:underline"><span className="ltr-nums font-mono">{m.matter.internalNumber}</span> · {m.matter.title}</a>
                            ) : (
                              <span className="ms-auto inline-flex items-center gap-1 text-meta text-ink-subtle"><Lock className="size-3" /> <span className="ltr-nums font-mono">{m.matter.internalNumber}</span> · {t("intake.restrictedMatter")}</span>
                            )
                          )}
                        </li>
                      ))}
          ...
          {current === "team" && (
            <div className="space-y-4">
              ...
              <div>
                <p className="mb-2 text-body font-medium">{t("intake.teamMembers")}</p>
              ...
              {conf === "HIGHLY_CONFIDENTIAL" && <p className="rounded-md bg-warning-soft px-3 py-2.5 text-meta text-warning">{t("intake.highlyConfidentialHint")}</p>}
            </div>
          )}
            ...
            <dl className="grid gap-x-6 gap-y-3 text-body sm:grid-cols-2">
              {[
                ...
                <div key={k} className="border-b border-line pb-2">
                  <dt className="text-meta text-ink-subtle">{k}</dt>
