...
export function RequestAccess({ matterId, pending }: { matterId: string; pending: boolean }) {
  ...
  if (sent) {
    return (
      <p className="inline-flex items-center gap-2 rounded-md bg-info-soft px-3 py-2 text-body text-info">
  ...
  return (
    <div className="mx-auto flex max-w-sm flex-col gap-3 text-start">
      <label htmlFor="reason" className="text-body font-medium text-ink">{t("workspace.requestReason")}</label>
