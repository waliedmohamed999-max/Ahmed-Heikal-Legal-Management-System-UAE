        ...
        <div className="mb-3 flex gap-1 rounded-lg border border-line bg-surface p-1 shadow-xs" role="tablist">
          {channels.map((c) => (
            <button key={c.key} role="tab" aria-selected={channel === c.key} type="button" onClick={() => { setChannel(c.key); setConfirm(false); }}
              className={cn("flex flex-1 items-center justify-center gap-1.5 rounded-md px-3 py-1.5 text-[13px] font-medium", channel === c.key ? "bg-brand text-brand-fg" : "text-ink-muted hover:bg-surface-muted")}>
              <c.icon className="size-4" /> {t(c.label)}
              <span className="text-[11px] opacity-70 tabular">{notes.filter((n) => n.visibility === c.key).length}</span>
            </button>
          ))}
        </div>
        <p className={cn("mb-3 flex items-center gap-1.5 text-[12.5px]", ch.tone)}><ch.icon className="size-3.5" /> {t(ch.hint)}</p>
        ...
        {canCreate && (channel !== "CLIENT" || canShare) && (
          <div className={cn("mb-4 rounded-lg border bg-surface p-3 shadow-xs", channel === "CLIENT" ? "border-warning/40" : "border-line")}>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-2 border-t border-line pt-2">
              {channel === "CLIENT" ? <Checkbox checked={confirm} onChange={(e) => setConfirm(e.target.checked)} label={<span className="text-[12.5px] text-warning">{t("workspace.confirmClient")}</span>} /> : <span />}
            ...
            {list.map((n) => (
              <li key={n.id} className={cn("group rounded-lg border bg-surface p-4 shadow-xs", n.pinned ? "border-accent/40" : "border-line")}>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-[13px] font-medium">{n.author}</span>
                  <span className="text-[12px] text-ink-subtle">· {relativeTime(n.createdAt, locale)}</span>
            ...
            {comments.map((c) => (
              <li key={c.id} className="flex gap-2.5 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[12.5px]"><span className="font-medium">{c.author}</span> <span className="text-ink-subtle">· {relativeTime(c.createdAt, locale)}</span></div>
                  <Body text={c.body} />
                </div>
              </li>
            ))}
