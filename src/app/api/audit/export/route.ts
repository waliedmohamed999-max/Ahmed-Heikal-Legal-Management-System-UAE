...
export const GET = staffRoute(async (req, ctx) => {
  ...
  const body = toCsv(header, rows.map((r) => [r.seq.toString(), r.createdAt.toISOString(), r.actor?.email, r.action, r.entityType, r.entityId, r.matter?.internalNumber, r.ip, r.userAgent, r.before, r.after, r.metadata, r.prevHash, r.hash]));
