...
export default async function ServicesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
      ...
      <p className="mt-2 max-w-2xl text-heading text-ink-muted">{t("site.practiceIntro")}</p>
        ...
        {areas.map((a) => (
          <li key={a.id}>
            <Link href={`/services/${a.slug}`} className="grid gap-2 py-6 hover:bg-surface-muted/60 sm:grid-cols-[1fr_2fr] sm:px-2">
              ...
              <span className="text-ui leading-relaxed text-ink-muted">{L(a.summaryEn, a.summaryAr)}</span>
            </Link>
          </li>
        ))}
      </ul>
