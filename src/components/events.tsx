...
export function EventTypeChip({ type, className }: { type: string; className?: string }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 text-meta font-medium", className)} style={{ color: s.color }}>
...
export function EventRow({ e, showDate, showCountdown = true }: { e: EventRowData; showDate?: boolean; showCountdown?: boolean }) {
  ...
  return (
    <Link href={e.href} className="group flex items-stretch gap-3 px-4 py-2 transition-colors hover:bg-surface-muted">
      <div className="w-[64px] shrink-0 pt-0.5 text-end">
        <div className="text-meta font-medium tabular text-ink">{formatTime(e.startsAt, locale, tz)}</div>
        {showDate && <div className="text-caption text-ink-subtle">{formatDate(e.startsAt, locale, tz, { day: "numeric", month: "short", year: undefined })}</div>}
      </div>
      <span aria-hidden className="w-[3px] shrink-0 rounded-full" style={{ background: s.color }} />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="bidi-plain truncate text-body font-medium text-ink group-hover:underline">{e.title}</span>
          {e.needsVerification && (
            <Badge tone="warning">
        ...
        <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-meta text-ink-muted">
          ...
          {e.matter && (
            <span className="truncate">
              <span className="ltr-nums font-mono text-meta text-ink-subtle">{e.matter.internalNumber}</span> · {locale === "ar" ? e.matter.titleAr || e.matter.title : e.matter.title}
            </span>
          )}
