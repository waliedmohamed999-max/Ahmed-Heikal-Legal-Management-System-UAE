  ...
  return (
    <DialogContent title={t("hearings.report")} description={`${hearing.title} — ${formatDateTime(hearing.startsAt, locale, tz)}`} size="lg">
      <form onSubmit={submit} className="grid gap-4" noValidate>
        <p className="flex items-start gap-2 rounded-md bg-info-soft px-3 py-2 text-[12.5px] text-info"><Info className="mt-0.5 size-4 shrink-0" /> {t("hearings.reportIntro")}</p>
        ...
        <fieldset className="grid gap-4 rounded-md border border-line p-3 sm:grid-cols-2">
          <legend className="px-1 text-[12px] font-semibold text-ink-muted">{t("hearings.next")}</legend>
        ...
        <fieldset className="grid gap-4 rounded-md border border-line p-3 sm:grid-cols-3">
          <legend className="px-1 text-[12px] font-semibold text-ink-muted">{t("hearings.nextDeadline")}</legend>
        ...
        <div>
          <p className="mb-2 text-[13px] font-medium">{t("hearings.followUpTasks")}</p>
          <div className="space-y-2">
            {tasks.fields.map((f, i) => (
              <div key={f.id} className="grid gap-2 sm:grid-cols-[1fr_200px_auto]">
